...
| [`exosat-rv/`](exosat-rv/) | Independently reproduces the first exosatellite radial-velocity detection (CD-35 2722 B) from public ESO data | **M0–M5 complete.** Verdict: the detection is **neither confirmed nor contradicted** — the pipeline built here reaches ~1850 m/s against a 246 m/s signal. That reading is only available because a **positive control** (GJ 229 B, a known 12.1-d binary) shows the pipeline does measure real velocities: Δχ² = 63.8 at the known period. Also: a verified-lossless ADP→cr2res converter that lets `viper` run on archive data, an alias analysis showing peak position cannot separate the 88/115-d candidates while significance can, and two of its own M0 claims retracted. 84 tests |

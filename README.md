...
| [`exosat-rv/`](exosat-rv/) | Independently reproduces the first exosatellite radial-velocity detection (CD-35 2722 B) from public ESO data | **M0 — archive kill-check complete.** The preprint's 20 CRIRES+ epochs are exactly the 20 public H-band nights, and 17 are already ESO-reduced, so the reproduction needs no telescope time and almost no re-reduction. Disproved an inherited Hill-radius value and put provenance tags on every published number. 23 tests |

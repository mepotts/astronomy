...
| [`exosat-rv/`](exosat-rv/) | Independently reproduces the first exosatellite radial-velocity detection (CD-35 2722 B) from public ESO data | **M0–M6 complete.** **The conclusion reproduces** — fitting the paper's own published RV table with an independent code recovers the 169-d signal above the 0.1% FAP level, prefers an ~88-d second satellite over 14/70/115 d, and matches K₂ to 0.1%. The *measurements* do not: re-deriving RVs from archive spectra reaches ~1850 m/s against the 31 m/s needed, a gap validated as real by a **positive control** (GJ 229 B, a known 12.1-d binary, Δχ² = 63.8). Also ships a lossless ADP→cr2res converter that lets `viper` run on archive data, and an alias analysis showing peak position cannot separate the 88/115-d candidates while model evidence can. Three of its own earlier claims retracted. 90 tests |

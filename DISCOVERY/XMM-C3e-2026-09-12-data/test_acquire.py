"""Synthetic two-GET continuation and isolated prior provenance checks."""
...
SPEC = importlib.util.spec_from_file_location("c3e", Path(__file__).with_name("acquire.py"))
...
class Response:
    def __init__(self, method, url, body, status=200, headers=None):
        ...
        self.values = headers if headers is not None else {"Content-Length": [str(len(body))], "ETag": ['"fixed"'], "Last-Modified": ["fixed-date"]}
...
class Tests(unittest.TestCase):
    def setUp(self):
        ...
        self.prior_heads = []
        for i, (slot, size) in enumerate(zip(self.slots, M.PRIOR_SIZES, strict=True), 1):
            original = {**slot, "slot": i, "method": "HEAD"}
            http = {"method": "HEAD", "url": slot["url"], "status": 200,
                    "headers": {"Content-Length": [str(size)], "ETag": ['"fixed"'], "Last-Modified": ["fixed-date"]}}
    ...
    def fixture(self):
        bodies = [self.compressed + b"\0" * (size - len(self.compressed)) for size in M.PRIOR_SIZES]
    ...
    def test_complete_two_get_worker_parent_replay(self):
        with self.fixture(), patch.object(M, "verify_binding"):
            self.assertEqual(M.worker(), 0)
            result = M.assess(0)
            self.assertEqual(result["status"], M.PASS)
            self.assertEqual([m for m, _ in self.calls], ["GET", "GET"])
            self.assertEqual(result["resource_usage"]["compressed_bytes"], 779072)
            self.assertEqual(result["resource_usage"]["expanded_bytes"], 2 * len(fits_bytes()))
            ...
            self.assertEqual(len(list((self.here / "products").iterdir())), 4)
            self.assertEqual(len(list((self.here / "headers").iterdir())), 2)
            (self.here / "products/orphan.bin").write_bytes(b"x")
            with self.assertRaisesRegex(ValueError, "STOP_PRODUCT_SET"):
                M.T.verify_product_set(M.get_plan(self.start))

    def test_first_failure_prevents_second_get(self):
        with self.fixture(), patch.object(M, "verify_binding"):
    ...
    def test_both_prior_validators_mandatory(self):
        for key in ("ETag", "Last-Modified"):
            invalid = copy.deepcopy(self.start)
            del invalid["prior_heads"][0]["http"]["headers"][key]
            with self.subTest(key=key), self.assertRaisesRegex(ValueError, "STOP_MISSING_PRIOR_VALIDATOR"):
                M.get_plan(invalid)

    def test_changed_last_modified_or_length_rejected_before_body(self):
        slot = M.get_plan(self.start)[0]
        for key, value in (("Last-Modified", ["changed"]), ("Content-Length", ["1"]), ("ETag", [])):
            with self.subTest(key=key), tempfile.TemporaryDirectory() as tmp, patch.object(M.T.C, "HERE", Path(tmp)):
                response = Response("GET", slot["url"], b"x", headers=copy.deepcopy(slot["head_headers"]))
                response.values[key] = value
                with patch("requests.Session", side_effect=lambda r=response: Session([r], self.calls)), self.assertRaises(ValueError):
                    M.T.C.download(slot)
                self.assertEqual(response.reads, [])

    def test_short_first_body_preserved_second_unattempted(self):
        with self.fixture(), patch.object(M, "verify_binding"):
            self.queue[0].stream = io.BytesIO(self.compressed)
            self.assertEqual(M.worker(), 1)
            result = M.assess(1)
        self.assertEqual(len(self.calls), 1)
        self.assertEqual([r["status"] for r in result["ledger"]], ["FAILED", "NOT_ATTEMPTED"])
        self.assertEqual(M.T.C.paths(self.slots[0])[0].stat().st_size, len(self.compressed))
        self.assertFalse(M.T.C.paths(self.slots[0])[1].exists())

    def test_prior_url_cannot_be_substituted(self):
        invalid = copy.deepcopy(self.start)
        invalid["prior_heads"][0]["http"]["url"] = self.slots[1]["url"]
    ...
    def test_isolated_modules_reduced_configuration_and_roundtrip(self):
        ...
        self.assertEqual(M.T.C.EXPANDED_TOTAL_CAP, 67108864)
        self.assertEqual(sum(M.T.C.EXPECTED), 779072)
        ...
        self.assertEqual(binding["configuration"]["expanded_total"], 67108864)
    ...
    def test_truncated_marker_timeout_preserves_both_slots(self):

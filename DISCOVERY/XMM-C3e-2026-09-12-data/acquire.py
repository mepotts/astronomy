"""Two exact-size map GETs from preserved C3 HEADs; no refresh or arrays."""
...
PROTOCOL = HERE.parent / "XMM-C3e-2026-09-12.md"
...
SECONDS, FREE_BYTES = 60, 134217728
...
P, T = load_prior("c3e_original_replay"), load_prior("c3e_current_helpers")
T.HERE = T.C.HERE = HERE
T.C.EXPECTED = PRIOR_SIZES
T.SECONDS = T.C.SECONDS = SECONDS
T.FREE_BYTES = FREE_BYTES
T.C.EXPANDED_TOTAL_CAP = 67108864
NAMES = T.NAMES[:2]
...
def plan():
    ...
    return [{"slot": n, "method": "GET", "url": T.O.BASE + name, "filename": name}
...
def get_plan(start):
    ...
    gets = []
    for slot, evidence in zip(slots, imported, strict=True):
        ...
        if any(not T.O.single(source_http["headers"], key) for key in ("ETag", "Last-Modified")):
            raise ValueError("STOP_MISSING_PRIOR_VALIDATOR")
        gets.append({**slot, "expected_bytes": receipt["advertised_compressed_bytes"], "head_headers": source_http["headers"]})
    if sum(g["expected_bytes"] for g in gets) != sum(PRIOR_SIZES):
        raise ValueError("STOP_COMPRESSED_TOTAL")
    return gets


def ledger(start, verify=True):
    rows, stopped, previous, gets = [], False, 0, get_plan(start)
        ...
        if marker.exists():
            ...
            if receipt.exists():
                ...
                if verify and record["status"] == "OK":
                    expected = T.product_record(slot, verify=True)
                    if record != expected:
                ...
                if record["status"] == "FAILED" and (set(record) != {"slot", "status", "error_type", "error_code"}
                        ...
                        or (record["error_code"] is not None and (not isinstance(record["error_code"], str)
                            or re.fullmatch(r"STOP_[A-Z0-9_]+", record["error_code"]) is None))):
...
def fallback_ledger():
    return [{"slot": n, "status": "UNVERIFIED_ATTEMPT" if any(HERE.glob(f"slot-{n}-*.json"))
             else "NOT_ATTEMPTED"} for n in range(1, 3)]
...
def worker():
    ...
    try:
        ...
        T.C.DEADLINE = None
        try:
            result["ledger"] = ledger(start, verify=False) if len(start["slots"]) == 2 else fallback_ledger()
        except Exception:
...
def run():
    ...
    try:
        ...
        helper = T.C.load_pinned("c3e_deadline", T.C.HELPER, T.C.HELPER_HASH)

...
* **A tracklet the MPC has already taken must not be re-submitted.** M12 measured the ITF
  draining at 4.4 departures per arrival; the queue goes stale underneath itself, and a
  full run over the real queue found exactly such a row.
...
def test_a_tracklet_the_mpc_has_already_taken_is_refused(sub):
    """The queue said STILL_LIVE; the current ITF disagrees, and the ITF wins."""
    triples, reasons = sub.check_row(_row(), {})
    assert triples == []
    assert sum("no longer in the ITF" in r for r in reasons) == 2   # both tracklets
...
# ----------------------------------------------------------------------------------
#
# Built only after reading /mpcops/documentation/identifications/additional/, which had
# been linked but never opened. Measured against the real queue it rejects 640 rows --

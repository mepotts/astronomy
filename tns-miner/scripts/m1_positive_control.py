...
def run(tns_csv: Path, label: str, limit: int | None = None) -> dict:
    ...
    for _, o in df.iterrows():
        ...
        try:
            ...
            oids.append(None)
            continue
        # Transport/service failures must abort the control, not masquerade as
        # an object with no Fink counterpart.
        oids.append(
            resolve_oid(
                s,
                ra,
    ...
    history_oids = [x for x in oids if x]
    ...
    for _, o in df.iterrows():
        oid = o["ztf_oid"]
        alerts = (
            pd.DataFrame(history_as_of(hist.get(oid, []), history_jd_ceiling))
        ...
        v_all = (
            F.evaluate(alerts, jd_cutoff=history_jd_ceiling)
            if len(alerts) else none_v
        )
        ...
        rows.append({
            ...
            "report_jd": rep,
            "history_jd_ceiling": history_jd_ceiling,
    ...
    summary = {
        ...
        "history_cache_provenance": cache_provenance(history_oids),
    }
